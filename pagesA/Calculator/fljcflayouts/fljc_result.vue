		...
		<view class="navigation-bar">
			<view class="sjfjsjg">
				<view class="sjftable">
					...
					<text class="text-blue">{{title}}防雷检测费计算结果</text>
				</view>
				<view class="cu-form-group">
					<view class="title">{{config.menuText[0]}}</view>
					<input name="input" class="sjfInput" v-model="result.fljcf_jj" /></input>
					<uni-tag text="元" type="defult"></uni-tag>
				</view>
				<view class="cu-form-group">
					<view class="title">{{config.menuText[1]}}</view>
					<input name="input" class="sjfInput" v-model="result.fljcf_zq" /></input>
					<uni-tag text="元" type="defult"></uni-tag>
				</view>
				<view class="cu-form-group">
					<view class="title">{{config.menuText[2]}}</view>
					<input name="input" class="sjfInput" v-model="result.fljcf_zh" /></input>
					<uni-tag text="元" type="defult"></uni-tag>
				</view>
	...
	export default {
		...
		data() {
			return {
				...
				showGroup: 1,
				config: {
					menuText: ['防雷设施检测费基价:', '折前防雷设施检测费:', '折后防雷设施检测费:', '计算过程:'],
				},
			};
		},
		created() {
