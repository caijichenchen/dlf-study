		...
		<view class="navigation-bar">
			<view class="sjfjsjg">
				<view class="sjftable">
					...
					<text class="text-blue">河北省工程建设材料试验质量检测费计算结果</text>
				</view>
				<view class="cu-form-group">
					<view class="title">{{config.menuText[0]}}</view>
					<input name="input" class="sjfInput" v-model="result.xcjchclsy_zq" /></input>
					<uni-tag text="元" type="defult"></uni-tag>
				</view>
				<view class="cu-form-group">
					<view class="title">{{config.menuText[1]}}</view>
					<input name="input" class="sjfInput" v-model="result.xcjchclsy_zh" /></input>
					<uni-tag text="元" type="defult"></uni-tag>
				</view>
	...
	export default {
		...
		data() {
			return {
				...
				showGroup: 1,
				config: {
					menuText: ['折前现场检测和材料试验收费:', '折后现场检测和材料试验收费:', '计算过程:'],
				},
			};
		},
		created() {
