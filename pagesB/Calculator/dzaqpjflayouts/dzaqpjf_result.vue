		...
		<view class="navigation-bar">
			<view class="sjfjsjg">
				<view class="sjftable">
					...
					<text class="text-blue">{{title}}地震安全评价费计算结果</text>
				</view>
				<view class="cu-form-group">
					<view class="title">{{config.menuText[0]}}</view>
					<input name="input" class="sjfInput" v-model="result.dzaqpjf_jj" /></input>
					<uni-tag text="元" type="defult"></uni-tag>
				</view>
				<view class="cu-form-group">
					<view class="title">{{config.menuText[1]}}</view>
					<input name="input" class="sjfInput" v-model="result.dzaqpjf_zq" /></input>
					<uni-tag text="元" type="defult"></uni-tag>
				</view>
				<view class="cu-form-group">
					<view class="title">{{config.menuText[2]}}</view>
					<input name="input" class="sjfInput" v-model="result.dzaqpjf_zh" /></input>
					<uni-tag text="元" type="defult"></uni-tag>
				</view>
	...
	export default {
		...
		data() {
			return {
				...
				showGroup: 1,
				config: {
					menuText: ['地震安全评价费基价:', '折前地震安全评价费:','折后地震安全评价费:','计算过程:'],
				},
			};
		},
		components: {
